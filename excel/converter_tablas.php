...
try {
	...
	$campos = array( 
		'profesiones_test'		=> array('cod', 'nombre_ppal', 'nombre_alt', 'descripcion'),
	...
	for ($row = 2; $row <= $highestRow; $row++) { 
	    ...
	    $insert_profesiones .= "'$cod','$nombre_ppal','$nombre_alt','$descripcion')";
	...
	for ($row = 2; $row <= $highestRow; $row++) { 
	    ...
	    //recoger datos del array rowData
	    $cod 						= $rowData[0][0];
	    $s_junior_min 				= str_replace(',', '.', $rowData[0][4]); 
	    $s_junior_max 				= str_replace(',', '.', $rowData[0][5]); 
	    $s_intermedio_min 			= str_replace(',', '.', $rowData[0][6]); 
	    $s_intermedio_max 			= str_replace(',', '.', $rowData[0][7]); 
	    $s_senior_min 				= str_replace(',', '.', $rowData[0][8]); 
	    $s_senior_max 				= str_replace(',', '.', $rowData[0][9]);
		$c_analisis 				= str_replace(',', '.', $rowData[0][10]);
		$c_comunicacio 				= str_replace(',', '.', $rowData[0][11]);
		$c_forma_fisica 			= str_replace(',', '.', $rowData[0][12]);
		$c_equipo 					= str_replace(',', '.', $rowData[0][13]);
		$c_organizacion 			= str_replace(',', '.', $rowData[0][14]);
		$i_ingles 					= str_replace(',', '.', $rowData[0][15]);
		$i_frances 					= str_replace(',', '.', $rowData[0][16]);
		$i_aleman 					= str_replace(',', '.', $rowData[0][17]);
		$i_otro 					= str_replace(',', '.', $rowData[0][18]);
		$i_otro_nombre 				= $rowData[0][19];
		$id_formacion_1 			= $rowData[0][20];
		$id_formacion_2 			= $rowData[0][21];
		$id_formacion_3 			= $rowData[0][22];
		$parados_enero_2014 		= str_replace(',', '.', $rowData[0][23]);
		$contratados_enero_2014 	= str_replace(',', '.', $rowData[0][24]);
		$parados_abril_2014 		= str_replace(',', '.', $rowData[0][25]);
		$contratados_abril_2014 	= str_replace(',', '.', $rowData[0][26]);
		$parados_julio_2014 		= str_replace(',', '.', $rowData[0][27]);
		$contratados_julio_2014 	= str_replace(',', '.', $rowData[0][28]);
		$parados_octubre_2014 		= str_replace(',', '.', $rowData[0][29]);
		$contratados_octubre_2014 	= str_replace(',', '.', $rowData[0][30]);
		$parados_enero_2015 		= str_replace(',', '.', $rowData[0][31]);
		$contratados_enero_2015 	= str_replace(',', '.', $rowData[0][32]);
		$parados_abril_2015 		= str_replace(',', '.', $rowData[0][33]);
		$contratados_abril_2015 	= str_replace(',', '.', $rowData[0][34]);
		$parados_julio_2015 		= str_replace(',', '.', $rowData[0][35]);
		$contratados_julio_2015 	= str_replace(',', '.', $rowData[0][36]);
		...
		foreach ($pdo->query($sql) as $row) {
			...
			$insert_empleabilidad 			.= "'$id_profesion','$parados',....)";
			$insert_capacidades 			.= "'$id_profesion','$c_memoria',....)";
			$insert_profesion_formacion 	.= "'$id_profesion','$id_formacion')"; // necesito las ids // primero consultar // o usar cods
