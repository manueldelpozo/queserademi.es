			...
			success: function(result) {
				...
				if (result == '[]' || result == '[""]') {
					...
					/*if (result.substring(result.length - $profesion.length - 2, result.length - 2) == $profesion) {
						...
						var urlEstatica = '/profesiones/' + $profesion.replace(/ /g,"-").latinize().toLowerCase() + '.html';
						//alert($(location).attr('protocol') + '//' + $(location).attr('hostname'));
						window.location = 'http://www.queserademi.es' + urlEstatica;
					}*/
					if (result.substring(result.length - $profesion.length - 2, result.length - 2) == $profesion) {
